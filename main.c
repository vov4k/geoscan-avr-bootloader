...
#define TIMER_ROUNDTRIPS_SECOND F_CPU / 1024 / 100
...
volatile int timeout_counter = TIMER_ROUNDTRIPS_SECOND;
...
void start_application(void)
{
	...
	// Отключение прерываний TIMER0_COMPA и таймера 0
	TCCR0A &= (1 << WGM01);
	TIMSK0 &= ~(1 << OCIE0A); // Отключение прерывания по совпадению
	TCCR0B = 0x00;			  // Остановка таймера 0
...
int main()
{
	...
	while (!upgrade_finished)
	{
		...
		if (err == 0)
		{
			...
			if (err <= 16 && page.ready)
			{ // this is a full, ok page
				program_page(&page);
				page_init_next(&page); // open up new page
				if (err > 0)
				{									   // no error, but not all bytes fittet on page
